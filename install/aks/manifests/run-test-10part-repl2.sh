...
for runnum in {1..25}; do
  ...
  sleep 10
	...
	kubectl delete -f sparkop-es-2.4.1-10part-repl1.yaml
  sleep 60 # Wait a min
  ...
  # Create Topic
  kubectl exec gateway-cp-kafka-0 --container cp-kafka-broker -- kafka-topics --zookeeper gateway-cp-zookeeper:2181 --topic ${topic} --create --replication-factor 2 --partitions 10
  ...
  kubectl apply -f sparkop-es-2.4.1-10part-repl1.yaml
	sleep 60 # Wait a min
